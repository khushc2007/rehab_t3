...
export default function ReportsPage() {
  ...
  const rows = ALL_SESSIONS_EXTENDED.filter(r => r.date.toLowerCase().includes(q.toLowerCase()) || r.exercise.toLowerCase().includes(q.toLowerCase())).slice().reverse()
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.date + r.time} className="border-b border-[#f3f4f6] dark:border-[#1a1a1a] last:border-0 hover:bg-[#f8fafc] dark:hover:bg-[#1a1a1a]/50">
                ...
                <td><Link href="/history" className="text-[#0F6E5E] text-[11px] px-4">View →</Link></td>
              </tr>
            ))}
